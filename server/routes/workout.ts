/**
 * Workout segment: exercise library, weekly plans (days + ordered exercises),
 * lifting sessions with set logging, per-exercise performance history, and
 * cardio (runs/walks) with step estimation.
 *
 * The exported helper functions below are the single data-access layer for
 * this segment — the workout agent (server/agents/defs/workout.ts) imports
 * and reuses them so routes and agent tools always agree.
 */
...
export function createFullPlan(input: FullPlanInput): PlanFull {
  ...
  const planId = db.transaction((): number => {
    const info = db
      .prepare("INSERT INTO workout_plans (name, description, goal) VALUES (?, ?, ?)")
      .run(name, optStr(input.description), optStr(input.goal));
