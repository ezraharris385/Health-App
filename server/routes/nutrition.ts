/**
 * Nutrition segment routes: food library CRUD, per-meal daily food log,
 * daily summary vs goals, day-aligned calorie/macro history, water tracking,
 * and daily weight log (one entry per day, upsert).
 *
 * The data helpers are exported so the nutrition agent's tools
 * (server/agents/defs/nutrition.ts) reuse the exact same validation and
 * write paths as the HTTP API.
 */
...
import type { Food, FoodLog, MealType, MicroMap, WaterLog, WeightLog } from "../../shared/types";
