/**
 * Vitamins segment routes: daily micronutrient coverage (from
 ...
 * adherence history.
 *
 * The data helpers are exported so the vitamins agent's tools
 * (server/agents/defs/vitamins.ts) reuse the exact same validation and
...
function parseDays(v: unknown, def: number, max = 365): number {
...
function parseId(v: unknown, field = "id"): number {
  const n = Number(v);
